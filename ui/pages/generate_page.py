...
class GeneratePage(QWidget):
    ...
    def _build_ui(self):
        ...
        self._source_viewer = ImageViewer(
            placeholder="拖放放图剧墖分版复刓n成点点鍑讳笅文规寜閽€择择",
        ...
        self._use_last_cb = QCheckBox("浣跨敤中婃结果")
        ...
        result_lbl = QLabel("生成成结果")
        result_lbl.setFont(Theme.font_title())
        rl.addWidget(result_lbl)
        self._result_viewer = ImageViewer(placeholder="生成成结果就析樉示出湪杩欓噷", max_size=500)
        ...
        self._open_btn = QPushButton("鎵取紑")
    ...
    def _on_image_selected(self, path: str):
        ...
        self._log.append(f"左查€择择: {Path(path).name}", "info")
    ...
    def _sync_copywriting(self):
        ...
        try:
            ...
            self._log.append(f"同步失败: {e}", "error")
    ...
    def _load_ai_prompt(self):
        ...
        try:
            ...
            if p.exists():
                ...
                self._log.append("鏈壘分癆I提愮示识嶏紝识峰厛鍦ㄦ枃妗堥〉生成成", "warning")
        except Exception as e:
            self._log.append(f"鍔犺浇失败: {e}", "error")
    ...
    def _start_generate(self):
        ...
        self._gen_btn.setText("生成成中?..")
        self._log.append(f"引€濮嬬敓成?[{self._engine_combo.currentText()}]", "step")
    ...
    def _on_done(self, path: str, gen_id: int):
        ...
        self._gen_btn.setIcon(Icons.palette())
        self._log.append(f"生成成瀹步成: {Path(path).name}", "success")

    def _on_error(self, err: str):
        self._log.append(f"閿欒: {err}", "error")
        self._gen_btn.setEnabled(True)
        self._gen_btn.setText("开始生成")
        self._gen_btn.setIcon(Icons.palette())
